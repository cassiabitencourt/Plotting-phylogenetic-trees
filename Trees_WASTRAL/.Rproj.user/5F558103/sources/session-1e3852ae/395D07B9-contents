...
# the quartet score and a final function to set up the plot of the phylogenetic tree. The end of this script is for loading the data. The tree that came from the summary method and a CSV file including two 
...
# --- 1 Define vectors of taxon groups ---
# Define rogue taxa
...
# --- Function QS_calc written by Sidonie Bellot ---
...
plot_tree_with_qpies <- function(AS_tree, title) {
  phylo <- AS_tree@phylo
  ...
  q_support <- QS_calc(AS_tree)
  pies <- nodepie(q_support, cols = 1:3, color = c(Q1 = '#006CD1', Q2 = '#994F00', Q3 = 'gray'))
  return(inset(base, pies, width = 0.06, height = 0.06))
}

# --- 3 Load data: terminal names and phylogenetic trees ---
...
AS_all_raxml_dt2 <- rename_tree("/Users/c.bitencourt/Documents/manuscripts/phylogenomics_Apocynaceae/Trees_WASTRAL/Data/dataset2_diamond/all_raxml_AAtoDNA/SpeciesTree_PxrrRooted_raxml_allebg.tre")
AS_nha_raxml_dt2 <- rename_tree("/Users/c.bitencourt/Documents/manuscripts/phylogenomics_Apocynaceae/Trees_WASTRAL/Data/dataset2_diamond/nha_raxml_AAtoDNA/SpeciesTree_PxrrRooted_raxml_nohard.tre")

# --- 4 Plot trees with Q pie charts and tip coloring ---
p_all_raxml_qs <- plot_tree_with_qpies(AS_all_raxml_dt2, "All genes (n=332)")
p_nha_raxml_qs <- plot_tree_with_qpies(AS_nha_raxml_dt2, "Only good genes (n=212)")

# --- 5 Export PDF ---
pdf("/Users/c.bitencourt/Documents/manuscripts/phylogenomics_Apocynaceae/Trees_WASTRAL/Results/Supplement_2_QSpies.pdf", 20, 50)
grid.arrange(p_all_raxml_qs, p_nha_raxml_qs, nrow = 1)
dev.off()
